...
//********* New Line Item Fields *****************
    ...
    /**
	 * To click Send request (Get Request)
	 * 
	 * @param screenShot
	 */
	public void clickSendRequestButton(boolean screenShot) {
		Log.event("Clicking 'Send request' button");
		(new WebDriverWait(driver, Duration.ofSeconds(30)).pollingEvery(Duration.ofMillis(200))
				.ignoring(NoSuchElementException.class, StaleElementReferenceException.class)
				.withMessage("Unable to find 'Send request' button"))
						.until(ExpectedConditions.visibilityOf(btnSendRequest));
		((JavascriptExecutor) driver).executeScript("arguments[0].click();", btnSendRequest);
		Log.message("Clicked 'Send request' button", driver, screenShot);
	}
	
	/**
	 * To click Launch Deep Link
	 * 
	 * @param screenShot
	 */
	public void clickLaunchDeepLink(boolean screenShot) {
		Log.event("Clicking 'Launch Deep Link'");
		RealizeUtils.scrollInToPage(driver, "Bottom");
		(new WebDriverWait(driver, Duration.ofSeconds(30)).pollingEvery(Duration.ofMillis(200))
				.ignoring(NoSuchElementException.class, StaleElementReferenceException.class)
				.withMessage("Unable to find 'Launch Deep Link' button"))
						.until(ExpectedConditions.visibilityOf(btnLaunchDeepLink));		
		((JavascriptExecutor) driver).executeScript("arguments[0].click();", btnLaunchDeepLink);
		Log.message("Clicked 'Launch Deep Link'", driver, screenShot);
	}
	
	/**
	 * To get JWT request body form IMS global page
	 * @return
	 */
	public String getJWTRequestBodyFromPlatform() {
		String jsonBody = "";
		if (RealizeUtils.waitForElement(driver, jsonBodyRequest, 30)) {
			jsonBody = RealizeUtils.getTextOfWebElement(jsonBodyRequest, driver);
		} else {
			Log.event("Unable to get Json body request");
		}
		return jsonBody;
	}
	
	/**
	 * To click Perform Launch
	 * 
	 * @param screenShot
	 */
	public void clickPerformLaunchButton(boolean screenShot) {
		Log.event("Clicking 'Perform Launch' button");
		(new WebDriverWait(driver, Duration.ofSeconds(30)).pollingEvery(Duration.ofMillis(200))
				.ignoring(NoSuchElementException.class, StaleElementReferenceException.class)
				.withMessage("Unable to find 'Perform Launch' button"))
						.until(ExpectedConditions.visibilityOf(btnPerformLaunch));
		RealizeUtils.scrollElementIntoMiddle(driver, btnPerformLaunch);		
		((JavascriptExecutor) driver).executeScript("arguments[0].click();", btnPerformLaunch);
		Log.message("Clicked 'Perform Launch' button", driver, screenShot);
	}
